---
title: "[Monthly report] 2020-09 my activity this month on GitHub"
date: 2020-09-30 23:31
public: true
qiita: false
author:
  name: CircleCI
  url: https://circleci.com/gh/sugarshin/build.blog.sugarshin.net/151
tags: monthly report
ogp:
  og:
    image:
      src: /assets/images/common/report/main.png
---

今月の主な OSS コントリビューションや GitHub 上のアクティビティまとめ

***

## Starred repositories

- [css-doodle/css-doodle](https://github.com/css-doodle/css-doodle)
- [rehooks/awesome-react-hooks](https://github.com/rehooks/awesome-react-hooks)
- [rehooks/local-storage](https://github.com/rehooks/local-storage)
- [jxnblk/mdx-deck](https://github.com/jxnblk/mdx-deck)
- [micromark/micromark](https://github.com/micromark/micromark)
- [remarkjs/strip-markdown](https://github.com/remarkjs/strip-markdown)
- [denolib/setup-deno](https://github.com/denolib/setup-deno)
- [mercari/production-readiness-checklist](https://github.com/mercari/production-readiness-checklist)
- [httpie/httpie](https://github.com/httpie/httpie)
- [git-up/GitUp](https://github.com/git-up/GitUp)
- [semantic-release/git](https://github.com/semantic-release/git)
- [x-motemen/git-pr-release](https://github.com/x-motemen/git-pr-release)
- [actions-rs/meta](https://github.com/actions-rs/meta)
